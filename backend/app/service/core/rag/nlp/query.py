...
class FulltextQueryer:
    ...
    def hybrid_similarity(self, avec, bvecs, atks, btkss, tkweight=0.3, vtweight=0.7):
        ...
        avec_arr = np.asarray(avec, dtype=float)
        bvecs_arr = np.asarray(bvecs, dtype=float)
        avec_norm = np.linalg.norm(avec_arr)
        bvecs_norm = np.linalg.norm(bvecs_arr, axis=1)
        denom = np.maximum(avec_norm * bvecs_norm, 1e-12)
        sims = np.dot(bvecs_arr, avec_arr) / denom
