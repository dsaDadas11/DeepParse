...
def naive_merge(sections, chunk_token_num=128, delimiter="\n。；！？"):
    ...
    def flush_current():
        ...
        chunk_text = "".join(part for part in current_parts if part).strip()
        if chunk_text:
    ...
    def sentence_fragments(text: str) -> list[str]:
        ...
        pieces = [piece.strip() for piece in re.split(split_pattern, normalized) if piece and piece.strip()]
        if len(pieces) <= 1:
        ...
        for piece in pieces:
            ...
            if pending and num_tokens_from_string(candidate) > chunk_token_num:
                fragments.append(pending.strip())
                pending = piece
            else:
                pending = candidate
        if pending.strip():
    ...
    def is_heading(text: str, layout: str) -> bool:
        ...
        if normalized.startswith("#"):
            return True
        if layout and re.search(r"title|head", str(layout), re.IGNORECASE):
            return True
        if re.search(r"[。；！？]$", normalized):
            return False
        heading_patterns = (
            r"^第[零一二三四五六七八九十百0-9]+(章|节|部分|条)",
            r"^[0-9]{1,2}(?:\.[0-9]{1,2}){0,3}[ 、.]",
            r"^[（(]?[一二三四五六七八九十0-9]+[)）][、.]?",
            r"^[一二三四五六七八九十]+[、.]",
            r"^(风险提示|风险因素|投资建议|盈利预测|主要财务指标|利润表|现金流量表|资产负债表)$",
        )
        return any(re.match(pattern, normalized) for pattern in heading_patterns)

    for sec, pos in sections:
        text = str(sec or "").strip()
        if not text:
            continue
        fragments = sentence_fragments(text)
        starts_new_chunk = is_heading(text, pos)
        for index, fragment in enumerate(fragments):
            fragment_tokens = num_tokens_from_string(fragment)
            if current_parts and (starts_new_chunk and index == 0):
                flush_current()
            if current_parts and current_tokens + fragment_tokens > chunk_token_num:
                flush_current()
